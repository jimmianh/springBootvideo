...
//                        join ...
//            cach 2:
//        List<DrgDrugUnit> listDrgDrugUnit []= jdbcTemplate.query("select * from employees", new EmployeeRowMapper(), drug_id)
//        List<DrgDrugPrice>  listDrgDrugPrice []= jdbcTemplate.query("select * from employees", new EmployeeRowMapper(), drug_id)
//                for(i... listDrgDrugUnit...i++) {
//                    for(j...listDrgDrugPrice...j++) {
//                        if(listDrgDrugPrice[j].drg_unit_id == listDrgDrugUnit[i].drg_unit_id) {
//                            return ...
//                        }
//            }
//        }
...
//    }
    ...
    public int insert(Drug drug) {
        return jdbcTemplate.update("INSERT INTO drg_inv (drug_id,drug_store_id, drg_drug_name, unit_name, price, quantity) " + "VALUE(?,?, ?, ?, ?,?)",
                drug.getDrug_id(), drug.getDrg_store_id(), drug.getDrg_drug_name(), drug.getUnit_name(), drug.getPrice(), drug.getQuantity());
    }

    public int updateByUnit(int drg_unit_id, BigDecimal price, String unit_name) {
        System.out.println("hello");
         jdbcTemplate.update("update drg_drug_price " + " set price = ? " + " where drg_unit_id = ?",
                new Object[] {
                        price, drg_unit_id
                });
        jdbcTemplate.update("update drg_drug_unit " + " set unit_name = ? " + " where drg_unit_id = ?",
                new Object[] {
                        unit_name, drg_unit_id
                });
        return 1;
    }
